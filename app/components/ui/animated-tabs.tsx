...
export const AnimatedTabs = ({ tabs, defaultTab, className, ariaLabel = "Tabs" }: AnimatedTabsProps) => {
	...
	return (
		<div className={cn("flex w-full min-w-0 flex-col gap-y-1", className)}>
			<div role="tablist" aria-label={ariaLabel} className="flex w-full min-w-0 flex-row gap-1 rounded-md border border-white/30 bg-black bg-opacity-50 backdrop-blur-sm sm:gap-2">
				{tabs.map((tab) => (
					<button
						key={tab.id}
						type="button"
