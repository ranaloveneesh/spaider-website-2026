...
export function RocketTrackMini() {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const k = Math.min(
        1,
        Math.max(0, (start - r.top) / Math.max(1, start - end)),
      );
      setT(k);
    };
    const onScroll = () => {
  ...
  return (
    <div ref={wrap} className="relative h-[130px] w-full text-muted-tertiary">
      <svg viewBox="0 0 560 160" className="absolute inset-0 h-full w-full">
        <path
          ref={pathRef}
          d="M60,80 C200,70 360,70 500,80"
          fill="none"
          stroke="currentColor"
          strokeWidth={6}
        ...
        />
        <g
          transform={`translate(${rocket.x},${rocket.y}) rotate(${rocket.r},${rocket.s / 2},${rocket.s / 2})`}
        >
          <path
            d={`M${rocket.s / 2} 5 
          ...
          />
          <path
            d={`M${rocket.s * 0.22} ${rocket.s * 0.6} L${rocket.s * 0.34} ${rocket.s * 0.54} L${rocket.s * 0.34} ${rocket.s * 0.78} L${rocket.s * 0.22} ${rocket.s * 0.84} Z`}
            fill="currentColor"
            stroke="currentColor"
            strokeWidth={1.1}
          />
          <path
            d={`M${rocket.s * 0.78} ${rocket.s * 0.6} L${rocket.s * 0.66} ${rocket.s * 0.54} L${rocket.s * 0.66} ${rocket.s * 0.78} L${rocket.s * 0.78} ${rocket.s * 0.84} Z`}
            fill="currentColor"
            stroke="currentColor"
            strokeWidth={1.1}
          />
      ...
      <div className="absolute left-1 top-1/2 -translate-y-1/2 text-center">
        ...
        <div className="mt-1 text-[12px] font-semibold text-muted-tertiary">
          On-cloud
        </div>
      </div>
