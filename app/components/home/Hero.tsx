...
export default function Hero() {
	return (
		...
		>
			{/* ── Background image ── */}
			<Image src="/space.png" alt="" fill priority className="object-cover object-[center_-10%]" sizes="100vw" />
			...
			<div
				...
				aria-hidden
				style={{
					background: "linear-gradient(to bottom, rgba(0,0,0,0.78) 0%, rgba(0,0,0,0.38) 55%, transparent 100%)",
				}}
			/>

			{/* ── Hero content ── */}
			<motion.div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 pt-36 pb-10 text-center" variants={heroContainer} initial="hidden" animate="show">
				{/* Headline - three lines stagger in with blur-to-sharp */}
				<motion.h1 variants={headlineContainer} className="font-outfit font-medium leading-[1] tracking-[-0.025em] text-white" style={{ fontSize: "clamp(3.5rem, 7vw, 6rem)", maxWidth: "16ch" }}>
				...
				{/* Sub-headline */}
				<motion.p variants={heroItem} className="mt-7 text-base leading-7 text-white/70 sm:text-lg sm:leading-8" style={{ maxWidth: "52ch" }}>
				...
				<motion.div variants={heroItem} className="mt-10 flex flex-wrap items-center justify-center gap-4">
					<CeramicButton href="/request-demo" color="#ffffff" textColor="#0a0a0b" ringColor="rgba(255,255,255,0.22)" borderRadius={8} padding="11px 24px" fontSize={13}>
						Request a Demo
					</CeramicButton>
					<CeramicButton href="/our-tech" color="rgba(255,255,255,0.05)" textColor="#ffffff" ringColor="rgba(255,255,255,0.14)" borderRadius={8} padding="11px 24px" fontSize={13}>
						Explore Our Tech
					</CeramicButton>
				</motion.div>
			</motion.div>
			...
			<motion.div className="relative z-10 w-full pb-14 pt-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, ease: "easeOut", delay: 0.8 }}>
				<p className="mb-6 text-center font-montserrat text-[10px] font-medium uppercase tracking-[0.2em] text-white/25">Trusted by industry leaders</p>
					...
					<div className="hero-marquee-track flex w-max items-center gap-x-6 sm:gap-x-10 md:gap-x-12 lg:gap-x-16 xl:gap-x-20">
						{MARQUEE_LOGOS.map(({ src, alt, scale, dup }) => (
							<div key={`${src}-${alt}-${dup}`} className="flex w-[132px] shrink-0 flex-col items-center gap-2 sm:w-[160px] md:w-[180px] lg:w-[200px]">
								...
								<span className="font-montserrat text-[10px] font-medium tracking-wide text-white/25">{alt}</span>
							</div>
						))}
					</div>
				</section>
