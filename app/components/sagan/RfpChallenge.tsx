...
export default function RfpChallenge() {
	...
	return (
		<section className="mx-auto mt-12 w-full min-w-0 sm:mt-16 md:mt-20 lg:mt-24">
			<h2 className="mb-6 font-manrope text-xl font-semibold tracking-tight text-foreground sm:mb-8 sm:text-2xl lg:text-3xl">
				The RFP Challenge
			</h2>
			<div className="flex flex-row justify-between gap-20">
				<Reveal variant="fade-left" threshold={0.25} className="max-w-md">
					<p className="mt-4 w-full text-xs leading-6 text-muted-foreground font-inter sm:mt-5 sm:text-sm sm:leading-7 lg:text-base lg:leading-7">Every year, space organizations handle hundreds of RFPs, each requiring significant time from specialized engineering teams.</p>

					<p className="mt-3 w-full text-xs leading-6 text-muted-foreground font-inter sm:text-sm sm:leading-7 md:mt-4 lg:text-base lg:leading-7">With limited resources and growing competition, teams face tough choices on where to invest effort—risking delays and missed opportunities.</p>
							...
							{items.map((c, i) => {
								...
								return (
									<div key={c.k} className="min-w-0 lg:border-l lg:border-[#ff595a]/15 lg:pl-6">
										...
										<h4 className="mt-3 font-manrope text-sm font-semibold text-foreground sm:text-base">{c.k}</h4>
										<p className="mt-1.5 text-xs leading-6 text-muted-foreground font-inter sm:text-sm sm:leading-7">{c.v}</p>
									</div>
								);
							})}
						</div>
