	...
	return (
		<section className="mx-auto mt-12 w-full min-w-0 sm:mt-16 md:mt-20 lg:mt-24">
			<div className="grid grid-cols-1 gap-8 md:grid-cols-12 md:gap-10">
				{/* LEFT: Copy */}
				<Reveal variant="fade-left" threshold={0.25} className="min-w-0 md:col-span-7">
					<h2 className="mb-6 font-manrope text-xl font-semibold tracking-tight text-foreground sm:mb-8 sm:text-2xl md:mb-10 lg:mb-12 lg:text-3xl">The RFP Challenge</h2>

					{/* P1 always visible */}
					<p className="mt-4 max-w-2xl text-xs leading-6 text-muted-foreground font-inter sm:mt-5 sm:text-sm sm:leading-7 lg:text-base lg:leading-7">Every year, space organizations handle hundreds of RFPs, each requiring significant time from specialized engineering teams.</p>

					{/* P2 collapsible on mobile, always visible on md+ */}
					<div className={`md:block ${expanded ? "block" : "hidden"}`}>
						<p className="mt-3 max-w-2xl text-xs leading-6 text-muted-foreground font-inter sm:text-sm sm:leading-7 md:mt-4 lg:text-base lg:leading-7">With limited resources and growing competition, teams face tough choices on where to invest effort—risking delays and missed opportunities.</p>
					</div>
				...
				<Reveal as="aside" variant="fade-right" threshold={0.25} delayMs={120} className="min-w-0 md:col-span-5" id="rfp-more">
					<div
						...
						style={{
							...
							background: "linear-gradient(135deg, rgba(255,89,90,0.14), rgba(255,255,255,0.03))",
						}}
					>
						<h3 className="font-manrope text-base font-semibold text-foreground sm:text-lg lg:text-xl">Common RFP Challenges</h3>

						<ul className="mt-4 space-y-2.5 sm:mt-5 sm:space-y-3 md:mt-6 md:space-y-5">
							{CHALLENGES.map((c, i) => (
								<li key={c.k} className={`${i >= 2 && !expanded ? "hidden md:flex" : "flex"} gap-2 text-xs leading-6 text-muted-foreground font-inter sm:gap-3 sm:text-sm sm:leading-7 md:gap-4 lg:text-base lg:leading-7`}>
									...
									<p>
										<span className="font-semibold text-white">{c.k}:</span> {c.v}
									</p>
								</li>
							))}
						</ul>
