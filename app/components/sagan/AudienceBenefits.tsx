...
export default function AudienceBenefits() {
  ...
  return (
    <section className="mx-auto w-[98%] max-w-[110rem] px-6 py-16">
      ...
      <div
        role="tablist"
        aria-label="Roles that benefit from SAGAN"
        className="mx-auto mt-6 flex w-full max-w-3xl items-center justify-between gap-2 rounded-full bg-white/5 p-1 ring-1 ring-white/10"
      >
        {tabs.map((t) => {
          ...
          return (
            <button
              ...
              className={[
                "flex-1 rounded-full px-4 py-2 text-sm font-semibold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#5ce1e6]",
        ...
        <ul className="space-y-5 text-white/90">
          {current.bullets.map((b, i) => (
            <li key={i} className="flex items-start gap-3">
              <CircleCheck
                className="h-6 w-6 shrink-0 text-[#5ce1e6] md:h-7 md:w-7"
                aria-hidden
              />
              <span>{b}</span>
            </li>
