				...
				{blogs.map((blog) => (
					<motion.div key={blog.href} variants={cardVariants} className="h-full">
						...
						>
							<div
								aria-hidden
								className="pointer-events-none absolute inset-0 bg-radial-[at_100%_100%] from-accent/15 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"
							/>
							{/* Cover image */}
							...
							<div className="flex flex-1 flex-col gap-2 p-4 sm:gap-2.5 sm:p-5 lg:p-6">
								<p className="font-montserrat text-[10px] font-medium uppercase tracking-[0.16em] text-muted-tertiary sm:text-[11px]">
									{blog.date}
								</p>
								<h2 className="font-outfit text-sm font-medium leading-snug tracking-tight text-foreground sm:text-base lg:text-lg">
									{blog.title}
								</h2>
								<p className="line-clamp-2 text-xs leading-5 text-muted sm:text-sm sm:leading-6 lg:line-clamp-3">
									{blog.excerpt}
								</p>
