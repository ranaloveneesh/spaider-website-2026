      ...
      <article className="space-y-0">
        ...
        <Section title="Result: 2nd Place in the OHB Challenge">
          ...
          <blockquote className="mt-4 rounded-xl border border-border bg-card p-4 sm:mt-6 sm:p-5">
            ...
            <footer className="mt-3 text-xs text-muted-tertiary font-inter">
              — Loveneesh Rana, CEO
            </footer>
          </blockquote>
        </Section>
        ...
        <div className="mt-8 flex flex-col gap-3 border-t border-border pt-6 sm:mt-10 sm:flex-row sm:flex-wrap sm:items-center sm:gap-3 sm:pt-8">
          ...
          <Link
            href="/invest"
            className="inline-flex min-h-9 w-full items-center justify-center rounded-full border border-border px-4 py-2 text-xs font-semibold text-foreground transition-colors hover:bg-muted/40 font-inter sm:min-h-10 sm:w-auto sm:px-5 sm:text-sm"
          >
            Invest
          </Link>
        </div>
...
function SubTitle({ children }: { children: ReactNode }) {
  return (
    <h3 className="mt-5 text-xs font-semibold text-foreground font-manrope first:mt-0 sm:mt-6 sm:text-sm lg:text-base">
      {children}
    </h3>
  );
}
