	...
	return (
		<div className="w-full min-w-0 space-y-6 pb-10 sm:space-y-8 sm:pb-12">
			...
			<Reveal as="header" variant="fade-up" threshold={0.35} className="space-y-3 sm:space-y-4">
				<h1 className="font-montserrat text-xl font-semibold tracking-tight text-foreground sm:text-2xl lg:text-3xl">We Secured 2nd Place at INNOspace Masters 2025 (And We’re Just Getting Started)</h1>

				<div className="flex flex-wrap items-center gap-2 text-sm text-muted font-inter">
					<span>{published}</span>
			...
			<article className="space-y-0">
				<Section title="Overview">
					...
					<p className="text-xs leading-6 text-foreground font-inter sm:text-sm sm:leading-7 lg:text-base lg:leading-7">We presented SPAIDER’s approach to intelligent automation for aerospace operations, joining a highly selective field and demonstrating how agentic workflows accelerate real programs.</p>
				</Section>
				...
				<Section title="Sagan - The AI RFP Manager">
					...
					<ul className="list-outside list-disc space-y-2 pl-4 text-xs leading-6 text-foreground font-inter sm:pl-5 sm:text-sm sm:leading-7 lg:text-base lg:leading-7">
						<li>
					...
					<p className="text-xs leading-6 text-foreground font-inter sm:text-sm sm:leading-7 lg:text-base lg:leading-7">Sagan integrates with enterprise collaboration and document systems so engineers can ask questions, request changes, and reference prior proposals without leaving the tools they already use.</p>
				</Section>

				<Section title="Result: 2nd Place in the OHB Challenge">
					<p className="text-xs leading-6 text-foreground font-inter sm:text-sm sm:leading-7 lg:text-base lg:leading-7">We were awarded 2nd place - a strong signal that the industry is ready for practical, high-impact AI in operations and that agents which understand real work can meaningfully accelerate it.</p>

					<blockquote className="mt-4 rounded-xl border border-border-card bg-car p-4 sm:mt-6 sm:p-5">
						<p className="text-sm font-semibold leading-snug text-foreground font-montserrat sm:text-base lg:text-lg">
						...
						<footer className="mt-3 text-xs text-muted font-inter">- Loveneesh Rana, CEO</footer>
					</blockquote>
				</Section>
...
function Section({ title, children }: { title: string; children: ReactNode }) {
	return (
		<Reveal as="section" variant="fade-up" threshold={0.2} className="mt-8 space-y-3 border-t border-border pt-6 first:mt-0 first:border-t-0 first:pt-0 sm:mt-10 sm:space-y-4 sm:pt-8">
			<h2 className="text-base font-semibold tracking-tight text-foreground sm:text-lg lg:text-xl font-montserrat">{title}</h2>
...
function SubTitle({ children }: { children: ReactNode }) {
	return <h3 className="mt-5 text-xs font-semibold text-foreground font-montserrat first:mt-0 sm:mt-6 sm:text-sm lg:text-base">{children}</h3>;
}
